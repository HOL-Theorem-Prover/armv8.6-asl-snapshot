...
open lem_pervasives_extra_sailTheory sail2_valuesTheory sail2_instr_kindsTheory;
...
val _ = type_abbrev((* ( 'a_regs, 'b_a, 'c_e) *) "monadS" , ``:'a_regs sequential_state -> (('b_a,'c_e)result #'a_regs sequential_state)``);

(*val returnS : forall 'regs 'a 'e. 'a -> monadS 'regs 'a 'e*)
val _ = Define `
 ((returnS:'a -> 'regs sequential_state ->('a,'e)result#'regs sequential_state) a s=  (Value a,s))`;


(*val bindS : forall 'regs 'a 'b 'e. monadS 'regs 'a 'e -> ('a -> monadS 'regs 'b 'e) -> monadS 'regs 'b 'e*)
val _ = Define `
 ((bindS:('regs sequential_state ->('a,'e)result#'regs sequential_state) ->('a -> 'regs sequential_state ->('b,'e)result#'regs sequential_state) -> 'regs sequential_state ->('b,'e)result#'regs sequential_state) m f (s : 'regs sequential_state)=
   ((\x .  (case x of   (Value a, s') => f a s' | (Ex e, s') => (Ex e, s') )) (m s)))`;


(*val seqS: forall 'regs 'b 'e. monadS 'regs unit 'e -> monadS 'regs 'b 'e -> monadS 'regs 'b 'e*)
val _ = Define `
 ((seqS:('regs sequential_state ->((unit),'e)result#'regs sequential_state) ->('regs sequential_state ->('b,'e)result#'regs sequential_state) -> 'regs sequential_state ->('b,'e)result#'regs sequential_state) m n=  (bindS m (\u .  
  (case (u ) of ( (_ : unit) ) => n ))))`;


(*val choose : forall 'a. list 'a -> 'a*)

(*val chooseS : forall 'regs 'a 'e. list 'a -> monadS 'regs 'a 'e*)
val _ = Define `
 ((chooseS:'a list -> 'regs sequential_state ->('a,'e)result#'regs sequential_state) l=  (returnS (@x. MEM x l)))`;


(*val readS : forall 'regs 'a 'e. (sequential_state 'regs -> 'a) -> monadS 'regs 'a 'e*)
val _ = Define `
 ((readS:('regs sequential_state -> 'a) -> 'regs sequential_state ->('a,'e)result#'regs sequential_state) f=  (\ s .  returnS (f s) s))`;


(*val updateS : forall 'regs 'e. (sequential_state 'regs -> sequential_state 'regs) -> monadS 'regs unit 'e*)
val _ = Define `
 ((updateS:('regs sequential_state -> 'regs sequential_state) -> 'regs sequential_state ->((unit),'e)result#'regs sequential_state) f=  (\ s .  returnS ()  (f s)))`;


(*val failS : forall 'regs 'a 'e. string -> monadS 'regs 'a 'e*)
val _ = Define `
 ((failS:string -> 'regs sequential_state ->('a,'e)result#'regs sequential_state) msg s=  (Ex (Failure msg), s))`;


(*val choose_boolS : forall 'regval 'regs 'a 'e. unit -> monadS 'regs bool 'e*)
val _ = Define `
 ((choose_boolS:unit -> 'regs sequential_state ->((bool),'e)result#'regs sequential_state) () =  (chooseS [F; T]))`;
...
val _ = Define `
 ((exitS:unit -> 'regs sequential_state ->('a,'e)result#'regs sequential_state) () =  (failS "exit"))`;


(*val throwS : forall 'regs 'a 'e. 'e -> monadS 'regs 'a 'e*)
val _ = Define `
 ((throwS:'e -> 'regs sequential_state ->('a,'e)result#'regs sequential_state) e s=  (Ex (Throw e), s))`;


(*val try_catchS : forall 'regs 'a 'e1 'e2. monadS 'regs 'a 'e1 -> ('e1 -> monadS 'regs 'a 'e2) ->  monadS 'regs 'a 'e2*)
val _ = Define `
 ((try_catchS:('regs sequential_state ->('a,'e1)result#'regs sequential_state) ->('e1 -> 'regs sequential_state ->('a,'e2)result#'regs sequential_state) -> 'regs sequential_state ->('a,'e2)result#'regs sequential_state) m h s=
   ((\x .  (case x of
...
val _ = Define `
 ((assert_expS:bool -> string -> 'regs sequential_state ->((unit),'e)result#'regs sequential_state) exp msg=  (if exp then returnS ()  else failS msg))`;
...
val _ = Define `
 ((early_returnS:'r -> 'regs sequential_state ->('a,(('r,'e)sum))result#'regs sequential_state) r=  (throwS (INL r)))`;


(*val catch_early_returnS : forall 'regs 'a 'e. monadRS 'regs 'a 'a 'e -> monadS 'regs 'a 'e*)
val _ = Define `
 ((catch_early_returnS:('regs sequential_state ->('a,(('a,'e)sum))result#'regs sequential_state) -> 'regs sequential_state ->('a,'e)result#'regs sequential_state) m=
   (try_catchS m
...
val _ = Define `
 ((liftRS:('regs sequential_state ->('a,'e)result#'regs sequential_state) -> 'regs sequential_state ->('a,(('r,'e)sum))result#'regs sequential_state) m=  (try_catchS m (\ e .  throwS (INR e))))`;


(* Catch exceptions in the presence of early returns *)
(*val try_catchRS : forall 'regs 'a 'r 'e1 'e2. monadRS 'regs 'a 'r 'e1 -> ('e1 -> monadRS 'regs 'a 'r 'e2) ->  monadRS 'regs 'a 'r 'e2*)
val _ = Define `
 ((try_catchRS:('regs sequential_state ->('a,(('r,'e1)sum))result#'regs sequential_state) ->('e1 -> 'regs sequential_state ->('a,(('r,'e2)sum))result#'regs sequential_state) -> 'regs sequential_state ->('a,(('r,'e2)sum))result#'regs sequential_state) m h=
   (try_catchS m
...
val _ = Define `
 ((maybe_failS:string -> 'a option -> 'regs sequential_state ->('a,'e)result#'regs sequential_state) msg= 
...
val _ = Define `
 ((read_memt_bytesS:read_kind -> num -> num -> 'regs sequential_state ->(((memory_byte)list#bitU),'e)result#'regs sequential_state) _ addr sz=  (bindS
...
val _ = Define `
 ((read_mem_bytesS:read_kind -> num -> num -> 'regs sequential_state ->(((memory_byte)list),'e)result#'regs sequential_state) rk addr sz=  (bindS
...
val _ = Define `
 ((write_memt_bytesS:write_kind -> num -> num ->(memory_byte)list -> bitU -> 'regs sequential_state ->((bool),'e)result#'regs sequential_state) _ addr sz v t=  (seqS
...
val _ = Define `
 ((write_mem_bytesS:write_kind -> num -> num ->(memory_byte)list -> 'regs sequential_state ->((bool),'e)result#'regs sequential_state) wk addr sz v=  (write_memt_bytesS wk addr sz v B0))`;
...
val _ = Define `
 ((read_regS:('regs,'rv,'a)register_ref -> 'regs sequential_state ->('a,'e)result#'regs sequential_state) reg=  (readS (\ s .  reg.read_from s.regstate)))`;
...
val _ = Define `
 ((read_regvalS:(string -> 'regs -> 'rv option)#(string -> 'rv -> 'regs -> 'regs option) -> string -> 'regs sequential_state ->('rv,'e)result#'regs sequential_state) (read, _) reg=  (bindS
...
val _ = Define `
 ((write_regvalS:(string -> 'regs -> 'rv option)#(string -> 'rv -> 'regs -> 'regs option) -> string -> 'rv -> 'regs sequential_state ->((unit),'e)result#'regs sequential_state) (_, write) reg v=  (bindS
...
val _ = Define `
 ((write_regS:('regs,'rv,'a)register_ref -> 'a -> 'regs sequential_state ->((unit),'e)result#'regs sequential_state) reg v=
