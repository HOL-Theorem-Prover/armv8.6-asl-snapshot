...
open lem_pervasives_extra_sailTheory lem_machine_wordTheory;
...
val _ = Define `
 ((dec_str:int -> string) bv=  (lem_string_extra_sail$stringFromInteger bv))`;
...
val _ = Define `
 ((decimal_string_of_bv:'a Bitvector_class -> 'a -> string)dict_Sail2_values_Bitvector_a bv=
  ...
  lem_string_extra_sail$stringFromInteger sum))`;
...
(*
  * sail2_valuesTheory.until_def : α -> (α -> bool) -> (α -> α) -> α
      until vars cond body = let vars' = body vars in
        if cond vars' then vars' else until (body vars') cond body
    *** TODO is the original definition wrong? It "steps" twice before checking cond... ***
    ⇒ redefine in terms of while:
*)
Definition until_alt_def[nocompute]:
  until vars cond body = while (body vars) ($~ o cond) body
End

Theorem until_def[compute]:
  ∀ vars cond body.
    until vars cond body = let vars' = body vars in
      if cond vars' then vars' else until vars' cond body
Proof
  rw[Once until_alt_def] >>
  rw[Once while_def] >>
  rw[GSYM until_alt_def]
QED
