...
val _ = new_theory "lem_list_extra_sail"
...
(*
  * lem_list_extraTheory.unfoldr_def : (α -> (β # α) option) -> α -> β list
      unfoldr f x = case f x of NONE => [] | SOME (y, x') => y :: unfoldr f x'
    Not used in the snapshot from the sail-arm repository
    ⇒ Does not appear to be well-formed, reformulate using lazy lists.
*)
Definition unfoldr_alt_def[nocompute]:
  unfoldr f x = LUNFOLD (OPTION_MAP SWAP o f) x
End

Theorem unfoldr_def[compute]:
  ∀ f x.
    unfoldr f x = case f x of NONE => [||] | SOME (y, x') => y ::: (unfoldr f x')
Proof
  rw[unfoldr_alt_def, Once LUNFOLD] >>
  Cases_on `f x` >> fs[] >>
  rename1 `SOME y` >> PairCases_on `y` >> fs[pairTheory.SWAP_def]
QED
