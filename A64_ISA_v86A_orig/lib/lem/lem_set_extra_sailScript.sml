...
val _ = new_theory "lem_set_extra_sail"
...
(*
  * lem_set_extraTheory.leastFixedPointUnbounded_def :
      (α set -> α set) -> α set -> α set
...
Theorem leastFixedPointUnbounded_def[compute]:
  ∀ f x . leastFixedPointUnbounded f x =
    if f x ⊆ x then x else leastFixedPointUnbounded f (f x ∪ x)
Proof
